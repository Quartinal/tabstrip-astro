...
for (const file of ls('./out/*.ts')) {
  ...
  const replacedFile = readFile.replace(
    "import '/strings.m.js'",
    "import './strings.m.js'",
  )
  ...
  for (const importedHtmlJsFile of importedHtmlJsFiles) {
    ...
    if (
      newReadFile.includes(`import {getTemplate} from '${importedHtmlJsFile}'`)
    ) {
      const replacedHtmlJsFile = newReadFile
        .replace(
          `import {getTemplate} from '${importedHtmlJsFile}'`,
          `import {getTemplateHtml} from '${importedHtmlJsFile}'`,
        )
        .replace('getTemplate()', 'getTemplateHtml()')
    ...
    if (newReadFile.includes('function(): HTMLTemplateElement')) {
      const replacedReadFile = newReadFile.replace(
